...
const BULB_COLORS = [
  ...
  '#ffd700', '#2255ff', '#ff6600', '#ff1a1a',
  '#00dd44', '#ffd700', '#2255ff', '#ff6600',
  '#cc00bb', '#ff1a1a', '#00dd44', '#ffd700',
  '#2255ff', '#ff6600', '#ff1a1a', '#00dd44',
];
...
export default function HangingBanner3D({ config, onComplete }: HangingBanner3DProps) {
  ...
  useEffect(() => {
    ...
    // Wait one frame so all refs are attached
    const raf = requestAnimationFrame(() => {
      ...
      if (!banner || !leftRope || !rightRope || !lightsWrap || !wrapper) return;

      // Make wrapper visible first
      gsap.set(wrapper, { autoAlpha: 1 });

      // Initial states
      gsap.set([leftRope, rightRope], { scaleY: 0, transformOrigin: 'top center' });
      gsap.set(banner, { y: -300, opacity: 0, rotationX: 35 });
  ...
  // Once the animation fully finishes, unmount
  if (gone) return null;
  ...
  return (
    <>
      ...
      {/* Outer wrapper — starts invisible, GSAP makes it visible */}
      <div
        ref={wrapperRef}
        className="fixed top-0 left-0 right-0 z-[9999] pointer-events-none"
        style={{ height: '280px', visibility: 'hidden' }}
      >
        ...
        <div ref={lightsWrapRef} style={{ position: 'relative', width: '100%', height: '40px' }}>
          ...
          <div style={{
            ...
            boxShadow: '0 1px 4px rgba(0,0,0,0.6)', zIndex: 1,
          }} />
          ...
          {Array.from({ length: bulbCount }).map((_, i) => {
            ...
            const animName = animVariant === 0 ? 'xmas-twinkle-a' : animVariant === 1 ? 'xmas-twinkle-b' : 'xmas-twinkle-c';
            const duration = 1.1 + (i % 5) * 0.28;
            const delay = (i * 0.11) % 1.8;
            return (
              <div key={i} style={{
                position: 'absolute', left: `${leftPct}%`, top: '3px',
                transform: 'translateX(-50%)',
                display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2,
              }}>
                <div style={{ width: '1.5px', height: '7px', background: '#444', flexShrink: 0 }} />
                <div style={{
        ...
        <div style={{
          ...
          <div ref={leftRopeRef} style={{
            ...
            width: '3px', height: '115px',
            background: 'repeating-linear-gradient(180deg, #6b3a1f 0px, #a0652a 6px, #6b3a1f 12px)',
            borderRadius: '2px', transformOrigin: 'top center',
            boxShadow: 'inset -1px 0 2px rgba(255,255,255,0.12), 2px 2px 6px rgba(0,0,0,0.45)',
          }} />

          {/* Banner text panel */}
          <div ref={bannerRef} className="pointer-events-auto" style={{
