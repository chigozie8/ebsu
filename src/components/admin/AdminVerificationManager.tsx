...
import { useAllUserProfiles, useToggleVerification } from '../../hooks/usePrivateChat';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Grant or revoke the verified badge for any user.
          </p>
        </div>
        ...
        {(['all', 'verified', 'unverified'] as const).map((f) => (
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all capitalize ${
              filter === f ? 'bg-[#25D366] text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            {f}
          </button>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-100">
            ...
            {filtered.map((profile) => {
              const gradient = getAvatarColor(profile.user_name);
              const ini = getInitials(profile.user_name);
                ...
                <div key={profile.user_id} className="grid grid-cols-12 gap-2 px-5 py-3.5 items-center hover:bg-gray-50/80 transition-colors">
                  ...
                  <div className="col-span-6 flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate mt-0.5">{profile.user_id.slice(0, 16)}…</p>
                    </div>
                  </div>
