        ...
        <div className="pt-[70px] xxss:pt-[80px] ss:pt-[90px] sm:pt-[105px] pb-6">
          ...
          >
            {[
              ...
              { id: "image", label: "Generate Image", icon: "M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" },
              { id: "vision", label: "Analyze Image", icon: "M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => handleTabChange(tab.id as "chat" | "image" | "vision")}
                className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 sm:py-3 rounded-lg text-xs sm:text-sm font-medium transition-all ${
                  activeTab === tab.id
              ...
              >
                ...
                <span className="hidden xxss:inline">{tab.label}</span>
              </button>
            ))}
          </motion.div>
          ...
          >
            <button
              ...
              className="w-full flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 bg-white rounded-xl shadow-sm border border-gray-200 hover:border-green2 transition-colors"
            >
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-green2/10 flex items-center justify-center">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5 text-green2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <div className="text-left">
                  <p className="text-xs sm:text-sm font-semibold text-gray-900">{selectedModel.name}</p>
                  <p className="text-xss sm:text-xs text-gray-500">{selectedModel.provider}</p>
                </div>
              </div>
              <svg className={`w-5 h-5 text-gray-400 transition-transform ${showModelSelector ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>

            {showModelSelector && (
              <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-200 max-h-64 overflow-y-auto">
                {filteredModels.map((model) => (
                  ...
                  >
                    ...
                    {selectedModel.id === model.id && (
                      <svg className="w-5 h-5 text-green2" fill="currentColor" viewBox="0 0 20 20">
          ...
          >
            {/* Messages */}
            <div className="h-[350px] sm:h-[400px] md:h-[450px] overflow-y-auto p-3 sm:p-4 space-y-4">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-4">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-green2/10 flex items-center justify-center mb-4">
                    <svg className="w-8 h-8 sm:w-10 sm:h-10 text-green2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                messages.map((message) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className={`text-xss mt-1 ${message.role === "user" ? "text-white/70" : "text-gray-400"}`}>
              ...
              <div className="flex items-end gap-2 sm:gap-3">
                ...
                {(activeTab === "vision") && (
                  <button
                    ...
                    className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"
                  >
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    rows={1}
                    className="w-full resize-none rounded-xl border border-gray-200 px-3 sm:px-4 py-2 sm:py-3 pr-12 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-green2 focus:border-transparent"
            ...
            {activeTab === "chat" && (
              <>
                <button
                  ...
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Circulatory System</p>
                  <p className="text-xss text-gray-500">Learn anatomy</p>
                </button>
                <button
                  onClick={() => setInputValue("What are the symptoms of malaria?")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Malaria Symptoms</p>
                  <p className="text-xss text-gray-500">Medical query</p>
                </button>
                <button
                  onClick={() => setInputValue("Help me understand pharmacokinetics")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Pharmacokinetics</p>
                  <p className="text-xss text-gray-500">Drug study</p>
                </button>
                <button
                  onClick={() => setInputValue("Quiz me on medical terminology")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Medical Quiz</p>
                  <p className="text-xss text-gray-500">Test yourself</p>
                </button>
              </>
            )}
            {activeTab === "image" && (
              <>
                <button
                  ...
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Heart Anatomy</p>
                  <p className="text-xss text-gray-500">Medical illustration</p>
                </button>
                <button
                  onClick={() => setInputValue("A medical student studying in a library")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Study Scene</p>
                  <p className="text-xss text-gray-500">Lifestyle image</p>
                </button>
                <button
                  onClick={() => setInputValue("Cross-section of a human brain with labeled parts")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Brain Diagram</p>
                  <p className="text-xss text-gray-500">Educational</p>
                </button>
                <button
                  onClick={() => setInputValue("A beautiful sunset over a medical campus")}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors"
                >
                  <p className="text-xs sm:text-sm font-medium text-gray-900">Campus View</p>
                  <p className="text-xss text-gray-500">Scenic image</p>
                </button>
              </>
            )}
            {activeTab === "vision" && (
              <>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="text-left p-3 bg-white rounded-xl border border-gray-200 hover:border-green2 transition-colors col-span-2 sm:col-span-4"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-green2/10 flex items-center justify-center">
                      <svg className="w-5 h-5 text-green2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-xs sm:text-sm font-medium text-gray-900">Upload an Image</p>
                      <p className="text-xss text-gray-500">Analyze X-rays, diagrams, notes, or any medical image</p>
                    </div>
                  </div>
                </button>
              </>
